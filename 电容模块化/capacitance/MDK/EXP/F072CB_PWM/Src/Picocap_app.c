...
{        
    ...
    InMemory_Read_MultiBytes((uint32_t)&(((PCap_DataConvert_Param *)PCAP_PARAM_BASE_ADDRESS)->CapMax),   //CapMax成员相对地址
                            (uint8_t *)&FilterParam->InputRangeMax, 
                            sizeof(FilterParam->InputRangeMax));      //电容满点值
    InMemory_Read_MultiBytes((uint32_t)&(((PCap_DataConvert_Param *)PCAP_PARAM_BASE_ADDRESS)->CapMin),   //CapMin成员相对地址
                                ...
                                uint32_t Cap_AD_Value)
{
    ...
    Rate = (float)(PCap_Result - DataConvert_Param->CapADMin) 
                    / (float)(DataConvert_Param->CapADMax - DataConvert_Param->CapADMin);
    ...
    if((0x0000 == DataConvert_Param->CapDALow) || (0x0000 == DataConvert_Param->CapDAHigh)
        || (0x0000 == DataConvert_Param->CapADLow) || (0x0000 == DataConvert_Param->CapADHigh))
    {
        Output_DA_Value = (uint16_t)(Rate * (DataConvert_Param->CapDAMax 
    ...
    {                 
        //AD值在零点AD以下
        if(Cap_AD_Value < DataConvert_Param->CapADMin)  
        {
            Output_DA_Value = DataConvert_Param->CapDAMin;
        }
        //AD值在零点AD和下刻度AD之间
        else if((Cap_AD_Value < DataConvert_Param->CapADLow) 
            && (Cap_AD_Value) >= DataConvert_Param->CapADMin)
        {
            Rate = (float)(Cap_AD_Value - DataConvert_Param->CapADMin)
                / (float)(DataConvert_Param->CapADLow - DataConvert_Param->CapADMin);
            Output_DA_Value = DataConvert_Param->CapDALow - DataConvert_Param->CapDAMin;
            Output_DA_Value = (uint16_t)(Rate * Output_DA_Value) 
                                            + DataConvert_Param->CapDAMin;
        }
        //AD值在下刻度AD值和上刻度AD值之间
        else if((Cap_AD_Value < DataConvert_Param->CapADHigh) 
            && (Cap_AD_Value >= DataConvert_Param->CapADLow))
        {
            Rate = (float)(Cap_AD_Value - DataConvert_Param->CapADLow) 
                / (float)(DataConvert_Param->CapADHigh - DataConvert_Param->CapADLow);
            Output_DA_Value = DataConvert_Param->CapDAHigh - DataConvert_Param->CapDALow;
            Output_DA_Value = (uint16_t)(Rate * Output_DA_Value) 
                                            + DataConvert_Param->CapDALow;
        }
        //AD值在上刻度AD值和满量程AD值之间
        else if((Cap_AD_Value < DataConvert_Param->CapADMax) 
            && (Cap_AD_Value >= DataConvert_Param->CapADHigh))
        {
            Rate = (float)(Cap_AD_Value - DataConvert_Param->CapADHigh) 
                / (float)(DataConvert_Param->CapADMax - DataConvert_Param->CapADHigh);
            Output_DA_Value = DataConvert_Param->CapDAMax - DataConvert_Param->CapDAHigh;
            Output_DA_Value = (uint16_t)(Rate * Output_DA_Value) 
        ...
        {
            Output_DA_Value = DataConvert_Param->CapDAMax;
        }
    } 
                                ...
                                uint32_t LiquidHeight)
{
    ...
    float Rate;
        ...
        //液位高度 < 0点高度
    if(LiquidHeight < DataConvert_Param->CapPWM_High0)
    {
        PCapPWM_OutValue = DataConvert_Param->CapPWM0;                                        
    }
    //液位高度在 0点 ~ 1点高度 之间
    if(LiquidHeight >= DataConvert_Param->CapPWM_High0 
        && LiquidHeight < DataConvert_Param->CapPWM_High1)
    {
        Rate = (float)((float)LiquidHeight / (DataConvert_Param->CapPWM_High1 - DataConvert_Param->CapPWM_High0));
        PCapPWM_OutValue = DataConvert_Param->CapPWM0 - DataConvert_Param->CapPWM1;
        PCapPWM_OutValue = DataConvert_Param->CapPWM0 - (uint16_t)(Rate * PCapPWM_OutValue);
                                        
    }
    //液位高度在 1点 ~ 2点高度 之间
    else if(LiquidHeight >= DataConvert_Param->CapPWM_High1 
            && LiquidHeight < DataConvert_Param->CapPWM_High2)
    {
        Rate = (float)(((float)LiquidHeight - (DataConvert_Param->CapPWM_High1 - DataConvert_Param->CapPWM_High0)) 
                / (DataConvert_Param->CapPWM_High2 - DataConvert_Param->CapPWM_High1));
        PCapPWM_OutValue = DataConvert_Param->CapPWM1 - DataConvert_Param->CapPWM2;
        PCapPWM_OutValue = DataConvert_Param->CapPWM1 - (uint16_t)(Rate * PCapPWM_OutValue);
    }
    //液位高度在 2点 ~ 3点高度 之间
    else if(LiquidHeight >= DataConvert_Param->CapPWM_High2 
            && LiquidHeight < DataConvert_Param->CapPWM_High3)
    {
        Rate = (float)(((float)LiquidHeight - (DataConvert_Param->CapPWM_High2 - DataConvert_Param->CapPWM_High0)) 
                / (DataConvert_Param->CapPWM_High3 - DataConvert_Param->CapPWM_High2));
        PCapPWM_OutValue = DataConvert_Param->CapPWM2 - DataConvert_Param->CapPWM3;
        PCapPWM_OutValue = DataConvert_Param->CapPWM2 - (uint16_t)(Rate * PCapPWM_OutValue);
    }   
    //液位高度在 3点 ~ 4点高度 之间
    else if(LiquidHeight >= DataConvert_Param->CapPWM_High3 
            && LiquidHeight < DataConvert_Param->CapPWM_High4)
    {
        Rate = (float)(((float)LiquidHeight - (DataConvert_Param->CapPWM_High3 - DataConvert_Param->CapPWM_High0)) 
                / (DataConvert_Param->CapPWM_High4 - DataConvert_Param->CapPWM_High3));
        PCapPWM_OutValue = DataConvert_Param->CapPWM3 - DataConvert_Param->CapPWM4;
        PCapPWM_OutValue = DataConvert_Param->CapPWM3 - (uint16_t)(Rate * PCapPWM_OutValue);
    } 
    //液位高度在 4点 ~ 5点高度 之间
    else if(LiquidHeight >= DataConvert_Param->CapPWM_High4
            && LiquidHeight < DataConvert_Param->CapPWM_High5)
    {
        Rate = (float)(((float)LiquidHeight - (DataConvert_Param->CapPWM_High4 - DataConvert_Param->CapPWM_High0)) 
                / (DataConvert_Param->CapPWM_High5 - DataConvert_Param->CapPWM_High4));
        PCapPWM_OutValue = DataConvert_Param->CapPWM4 - DataConvert_Param->CapPWM5;
        PCapPWM_OutValue = DataConvert_Param->CapPWM4 - (uint16_t)(Rate * PCapPWM_OutValue);
    } 
    //液位高度 >= 5点高度
    else if(LiquidHeight >= DataConvert_Param->CapPWM_High5)
    {
        PCapPWM_OutValue = DataConvert_Param->CapPWM5;
    } 
