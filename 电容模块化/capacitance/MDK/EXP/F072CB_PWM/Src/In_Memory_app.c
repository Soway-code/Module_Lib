...
void InMemory_SystemParam_Check(void)
{
    ...
    {
        //备份1
        Cur_Param_Bak1 = InMemory_Read_OneByte((MODBUS_PARAM_BAK1_BASE_ADDRESS + Cnt));
        //设备参数与备份1不同
        if(InMemory_Read_OneByte((MODBUS_PARAM_BASE_ADDRESS + Cnt)) != Cur_Param_Bak1)
        {
            Check_Sta = 1;
        }
        //备份1有误
    ...
    if(Check_Sta == 0 && InMemory_Read_OneByte(SYSPARAM_FLAG_BAK1_PROGRAMED) != SYSTEMPARAM_IS_PROGRAMED)
    {
        Check_Sta = 1;
    }
    //如果数据有误就写入缺省系统参数
...
uint8_t InMemory_Write2T_MultiBytes(uint16_t FlashAddr, const uint8_t *pWrbuf, uint16_t Wrlen)
{
    ...
    //系统参数备份区1
    Wrsta = InMemory_Write_MultiBytes(FlashAddr + MODBUS_PARAM_BAK1_BASE_ADDRESS, pWrbuf, Wrlen);
    if(OP_SUCCESS != Wrsta)
    {
        Err = OP_FAILED;
    }
