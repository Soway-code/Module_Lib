...
typedef struct
{
	uint16_t CapMin;                                                                   //电容零点
	int32_t CapMax;                                                                   //电容满度
	uint32_t CapRange;                                                                 //电容范围       
	uint16_t Range;                                                                    //量程
	uint16_t SensorLen;                                                                 //传感器长度
	uint16_t OrignOffset;                                                              //零点偏移量
	float DAMax;                                                                  //DA满度
	...
	uint16_t UpThreshold;														  	  //每15s可上升的阈值
	uint16_t DownThreshold;															  //每15s可下降的阈值
...
typedef struct
{
	...
	uint32_t uartInitTime;                                                         //Uart初始化时间
	bool bInitializing;
}RunVarTypeDef;
...
typedef struct
{
  ...
  int32_t HFil[10];                                                                 //高阶滤波数组
  int32_t HFilBak[10];                                                              //高阶滤波数组备份
  int32_t LFil[96];                                                                 //低阶滤波数组
  int32_t LFilBak[96];                                                              //低阶滤波数组备份        
  int32_t PCap_Filter;                                                              //滤波后的电容值
