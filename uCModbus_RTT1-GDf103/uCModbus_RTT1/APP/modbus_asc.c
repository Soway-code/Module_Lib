...
//**************************************************************************************************
#define AutoUpLoadRegisterNum       12                                           //自动上传数据 寄存器个数*2  
void MBASC_AutoUpLoadFrame(void)
{
    ...
    SendBuf[SendLen++] = AutoUpLoadRegisterNum*2;		                //寄存器数量*2
     
    RunVarVaue = (u32)g_cyWorkState;                                            //此版为新算法，用不到此值
    for (i = 0; i < 4; i++)
        SendBuf[SendLen++] = (u8)(RunVarVaue >> ((3 - i) * 8));
    
    RunVarVaue = (u32)g_cyRecordShovelNum;                                      //此版为新算法，用不到此值
    for (i = 0; i < 4; i++)     
        SendBuf[SendLen++] = (u8)(RunVarVaue >> ((3 - i) * 8));

    RunVarVaue = (u32)0;                                                        //车辆载荷重量（暂无）
    for (i = 0; i < 4; i++)
        SendBuf[SendLen++] = (u8)(RunVarVaue >> ((3 - i) * 8));
        
    RunVarVaue = (u32)0;                                                        //载重相对值（暂无）
    for (i = 0; i < 4; i++)
        SendBuf[SendLen++] = (u8)(RunVarVaue >> ((3 - i) * 8));
    
    RunVarVaue = (u32)g_wRADValue;                                              //原始AD值
    for (i = 0; i < 4; i++)
        SendBuf[SendLen++] = (u8)(RunVarVaue >> ((3 - i) * 8));

    RunVarVaue = (u32)myOutData.Weight;                                         //浮动零点
    for (i = 0; i < 4; i++)
        SendBuf[SendLen++] = (u8)(RunVarVaue >> ((3 - i) * 8));
