...
const uint8_t Company_Name[] = {8,'S','o','W','a','y'};
const uint8_t Product_Code[] = {20,'S','F','C','G','2','0','L','-','1','1','3','5','A',
                                '-','M','2','-','M','E','P'};
const uint8_t Hardware_Version[] = {7,'H','V','1','.','0','.','0'};
const uint8_t Software_Version[] = {11,'S','V','1','.','1','.','1','5'};		//Èí¼þ°æ±¾
const uint8_t Device_ID[] = {8,'2','0','1','1','C','0','0','1'};
...
#endif
static  CPU_BOOLEAN          MBS_FC2B_FileWr               (MODBUS_CH *pch);
...
#endif
//   case 0x10:
//              send_reply = MBS_FC10_writes(pch);        /* Process loopback command.                                */
//                 break;
//			
#if (MODBUS_CFG_FC15_EN == DEF_ENABLED)
			...
			case MODBUS_FC2B_FILE_R:
				 if (pch->WrEn == DEF_TRUE) {
					 send_reply = MBS_FC2B_FileWr(pch);
				 } else {
					 send_reply = DEF_FALSE;
				 }
				 break;
...
static  CPU_BOOLEAN  MBS_FC2B_FileWr (MODBUS_CH  *pch)
{
    ...
    if (reg < MODBUS_CFG_FP_START_IX) {                          /* See if we want integer registers                         */
        ...
        if (nbr_regs == 0 || nbr_regs < 256) {                    /* Make sure we don't exceed the allowed limit per request  */
            pch->Err = MODBUS_ERR_FC04_04;
            MBS_ErrRespSet(pch, 
                           MODBUS_ERR_ILLEGAL_DATA_QTY);
            return (DEF_TRUE);                                   /* Tell caller that we need to send a response              */
    ...
    *presp++              = (CPU_INT08U)nbr_bytes;               /* Set number of data bytes in response message             */
    while (nbr_regs > 0) {                                       /* Loop through each register requested.                    */
        if (reg < MODBUS_CFG_FP_START_IX && ((reg & 0x00FF) == 0xE3)) 
		{                      /* See if we want an integer register                       */

			pch->TxFrameNDataBytes = nbr_bytes + Software_Version[0];                      /* Number of data bytes + byte count.        */
				*(presp+=4);                            
			for(i=1;i<9;i++)
			{
				*presp++ = Software_Version[i];
			}
  
        } 
		else if(reg < MODBUS_CFG_FP_START_IX && ((reg & 0x00FF) == 0xE0)) 
		{
			pch->TxFrameNDataBytes = nbr_bytes + Company_Name[0];                      /* Number of data bytes + byte count.        */
				*(presp+=4);                            
			for(i=1;i<6;i++)
			{
				*presp++ = Company_Name[i];
			}
		}
		else {
#if (MODBUS_CFG_FP_EN == DEF_ENABLED)
